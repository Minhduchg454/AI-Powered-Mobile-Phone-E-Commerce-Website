...
exports.createStatusUser = asyncHandler(async (req, res) => {
  ...
  if (!statusUserName)
    return res
      .status(400)
      .json({ success: false, mes: "Missing statusUserName" });
  // Kiểm tra trùng tên
  const existed = await StatusUser.findOne({ statusUserName });
  if (existed)
    return res
      .status(400)
      .json({ success: false, mes: "StatusUser already exists" });
  // Tạo mới
...
exports.updateStatusUser = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const { statusUserName } = req.body;
  const updated = await StatusUser.findByIdAndUpdate(
    id,
    { statusUserName },
    { new: true }
  );
  return res.json({
    success: !!updated,
    statusUser: updated || "Update failed",
  });
});
...
exports.deleteStatusUser = asyncHandler(async (req, res) => {
  ...
  return res.json({
    success: !!deleted,
    mes: deleted ? "StatusUser deleted" : "Delete failed",
  });
});
